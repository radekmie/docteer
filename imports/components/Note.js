    ...
    return (
      <dl class="h-100 ma0 overflow-auto pa3 w-100">
        {props.user.schemas.length > 1 && (
          <select
            class={`b--dark-gray ba bg-white br-0 bw1 h2 mb1${props.edit ? ' pointer' : ''} tc w-100`}
            disabled={!props.edit}
...
function listToSteps (html) {
  return html.indexOf('<li>') === 0 ? html.split('<li>').slice(1).map(element => element.slice(0, -5)) : html ? [html] : [];
}
